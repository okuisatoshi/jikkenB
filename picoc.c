...
int token;                           // 最後に読んだトークンの識別番号
char name[MAX_TK_LEN];               // ...の名前
int vsuffix = 1;                     // 仮想レジスタ名の接尾番号
int lsuffix = 1;                     // ラベルの接尾番号
struct hashmap *hmap;                // 変数名と行番号の対応を保存するハッシュ表
#define MAX_TMP_LEN (MAX_TK_LEN + 1) // 名前バッファの最大長
