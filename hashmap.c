...
struct hashmap *hnew(void) {
    return hashmap_new(sizeof(struct item), 0, 0, 0, item_hash, item_compare, item_free, NULL);
}

void hinsert(struct hashmap *hmap, const char *key, int value) {
    struct item kv = {key, value}; // kvはhashmap_set()内でコピーされる
    hashmap_set(hmap, &kv);
}

int hget(struct hashmap *hmap, const char *key) {
    struct item kv = {key};
